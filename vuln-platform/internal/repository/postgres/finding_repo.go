...
func (r *FindingRepo) Get(ctx context.Context, id string) (*entity.ScannerFinding, error) {
	...
	if err := r.populateExtracted(ctx, []*entity.ScannerFinding{f}); err != nil {
		return nil, fmt.Errorf("populate extracted identifiers for finding %s: %w", id, err)
	}
	return f, nil
}

// List supports dynamic filtering (severity, status, host, RHSA/CVE/
// package membership via the join tables, free-text search) with
// offset pagination, and populates each finding's extracted CVE/RHSA/
// package identifiers via populateExtracted below.
...
func (r *FindingRepo) List(ctx context.Context, f repository.FindingFilter) ([]*entity.ScannerFinding, int, error) {
	...
	if err := r.populateExtracted(ctx, out); err != nil {
		return nil, 0, fmt.Errorf("populate extracted identifiers: %w", err)
	}

	return out, total, nil
}

// populateExtracted fills in ExtractedCVEs/ExtractedRHSAs/ExtractedPackages
// on each finding from the finding_cves/finding_rhsas/finding_packages
// join tables, in three batched queries (WHERE finding_id = ANY($1))
// regardless of how many findings are passed in — avoiding an N+1
// query per finding. Callers (Get, List) already have the finding IDs
// in hand; this only needs to run the batched joins and assign
// results back onto the matching *entity.ScannerFinding by ID.
func (r *FindingRepo) populateExtracted(ctx context.Context, findings []*entity.ScannerFinding) error {
	if len(findings) == 0 {
		return nil
	}

	byID := make(map[string]*entity.ScannerFinding, len(findings))
	...
	cveRows, err := r.pool.Query(ctx, `SELECT finding_id, cve_id FROM finding_cves WHERE finding_id = ANY($1)`, ids)
	if err != nil {
		return fmt.Errorf("query finding_cves: %w", err)
	}
	for cveRows.Next() {
		var findingID, cveID string
		if err := cveRows.Scan(&findingID, &cveID); err != nil {
			cveRows.Close()
			return err
		}
		if f, ok := byID[findingID]; ok {
			f.ExtractedCVEs = append(f.ExtractedCVEs, cveID)
		}
	}
	cveRows.Close()
	...
	rhsaRows, err := r.pool.Query(ctx, `SELECT finding_id, rhsa_id FROM finding_rhsas WHERE finding_id = ANY($1)`, ids)
	if err != nil {
		return fmt.Errorf("query finding_rhsas: %w", err)
	}
	for rhsaRows.Next() {
		var findingID, rhsaID string
		if err := rhsaRows.Scan(&findingID, &rhsaID); err != nil {
			rhsaRows.Close()
			return err
		}
		if f, ok := byID[findingID]; ok {
			f.ExtractedRHSAs = append(f.ExtractedRHSAs, rhsaID)
		}
	}
	rhsaRows.Close()
	if err := rhsaRows.Err(); err != nil {
		return err
	}

	pkgRows, err := r.pool.Query(ctx, `SELECT finding_id, package_name FROM finding_packages WHERE finding_id = ANY($1)`, ids)
	if err != nil {
		return fmt.Errorf("query finding_packages: %w", err)
	}
	for pkgRows.Next() {
		var findingID, pkg string
		if err := pkgRows.Scan(&findingID, &pkg); err != nil {
			pkgRows.Close()
			return err
		}
		if f, ok := byID[findingID]; ok {
			f.ExtractedPackages = append(f.ExtractedPackages, pkg)
		}
	}
	pkgRows.Close()
	return pkgRows.Err()
...
func (r *FindingRepo) UnresolvedForCorrelation(ctx context.Context, batchSize int, cursor string) ([]*entity.ScannerFinding, string, error) {
	...
	if err := r.populateExtracted(ctx, out); err != nil {
		return nil, "", fmt.Errorf("populate extracted identifiers: %w", err)
	}
